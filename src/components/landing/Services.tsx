...
export const Services = () => {
  ...
  return (
    <section id="services" className="py-24 bg-premium-luxury-gradient bg-grid-landeros border-b border-black/5 font-sans-landeros text-[#0A0A0A] relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-7xl mx-auto">
          {agents.map((agent, index) => {
            ...
            return (
              <div 
                key={index} 
                className="bg-[#F8F8F8] border border-black/5 p-8 rounded-3xl flex flex-col justify-between relative group overflow-hidden hover-premium-card z-10"
              >
                {/* Backdrop Glow */}
                <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-black/5 to-transparent rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
 
                {/* Massive Number Stamp */}
                <div className="absolute top-4 right-6 font-display-landeros text-6xl md:text-8xl font-extrabold text-black/5 select-none transition-colors group-hover:text-black/8">
                  {agent.num}
                </div>
 
                <div>
                  {/* Icon Block */}
                  <div className="w-14 h-14 bg-black/5 border border-black/8 rounded-2xl flex items-center justify-center mb-6 text-[#0A0A0A] group-hover:bg-[#0A0A0A] group-hover:text-white transition-all duration-300 shadow-sm">
                    <Icon className="w-6 h-6 stroke-[2]" />
                  </div>
 
                  {/* Title */}
                  <h3 className="font-tech-landeros text-xl md:text-2xl font-bold mb-4 text-[#0A0A0A] group-hover:text-[#3A3A3A] transition-colors">
                    {agent.title}
                  </h3>
 
                  {/* Description */}
                  <p className="font-semibold text-sm leading-relaxed text-[#6B6B6B] mb-8 max-w-md">
                    {agent.description}
                  </p>
                </div>
                ...
                <div className="border-t border-black/5 pt-4 mt-auto">
                  <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-black/5 border border-black/10 font-bold uppercase tracking-wider text-xs text-[#0A0A0A] group-hover:border-black/20 transition-all duration-300">
                    <Zap className="w-3.5 h-3.5 fill-black/20 text-[#0A0A0A]" />
                    {agent.value}
                  </div>
                </div>
        ...
        <div className="mt-20 bg-[#F8F8F8] border border-black/5 p-8 shadow-landeros rounded-3xl max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 relative overflow-hidden">
